...
# WRITE YOUR SOLUTION BELOW:
BALANCE=BALANCE=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getbalance)
...
SEGWIT_ADDR=P2SH_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getnewaddress "" p2sh-segwit)
...
# WRITE YOUR SOLUTION BELOW:
P2SH_VALID=$(bitcoin-cli -regtest -rpcwallet=btrustwallet validateaddress "$P2SH_ADDR" | grep -o 'true\|false')
...
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$(echo "$ADDR_INFO" | grep -oP '(?<="internalkey": ")[^"]+')
...
# WRITE YOUR SOLUTION BELOW:
TAPROOT_DESCRIPTOR=$(echo "$TAPROOT_DESCRIPTOR" | tr -d ' \n\t')
...
# WRITE YOUR SOLUTION BELOW:
DERIVED_ADDR_RAW=$(echo "$DERIVED_ADDR_RAW" | tr -d '[]" \n\t')
